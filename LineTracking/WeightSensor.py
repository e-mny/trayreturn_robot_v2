...
class HX711:
    ...
    def read_average(self):
        ...
        for i in range(10): # affects checking intervals 
        ...
        final_avg = numpy.mean(numpy.array(final_list), axis = 0)
        return final_avg
        sum = 0
        for i in range(times):
            sum += self.read()
            print((self.read() - self.OFFSET) / self.SCALE)
        return sum / times        


    def get_grams(self, times=10):
        """
    ...
    #def final_average(self, times=5):
        arr = []

        for i in range(7): # affects checking intervals 
            value = (self.read() - self.OFFSET)
            grams = (value / self.SCALE)
            arr.append(grams)
